<template lang="jade">
div.container.questionListContainer
    div.export-graph
        div.row-top.row
            div.row--rtl
                //- img.t11(:src='test[current]', @click='current = current ? 0 : 1')
                img.t11(:src='preload', v-img='test[current]', @click='current = current ? 0 : 1')
                //- img.t12(src='/imgs/teeth/12/01.png')
                img.t12(:src='test2[current2]', @click='current2 = current2 ? 0 : 1')
                img.t13(:src='preload', v-img='"/imgs/teeth/13/01.png"')
                img.t14(:src='preload', v-img='"/imgs/teeth/14/01.png"')
                img.t15(:src='preload', v-img='"/imgs/teeth/15/01.png"')
                img.t16(:src='preload', v-img='"/imgs/teeth/16/01.png"')
                img.t17(:src='preload', v-img='"/imgs/teeth/17/01.png"')
                img.t18(:src='preload', v-img='"/imgs/teeth/18/01.png"')
            div.row--ltr
                img.t21(:src='preload', v-img='"/imgs/teeth/21/01.png"')
                img.t22(:src='preload', v-img='"/imgs/teeth/22/01.png"')
                img.t23(:src='preload', v-img='"/imgs/teeth/23/01.png"')
                img.t24(:src='preload', v-img='"/imgs/teeth/24/01.png"')
                img.t25(:src='preload', v-img='"/imgs/teeth/25/01.png"')
                img.t26(:src='preload', v-img='"/imgs/teeth/26/01.png"')
                img.t27(:src='preload', v-img='"/imgs/teeth/27/01.png"')
                img.t28(:src='preload', v-img='"/imgs/teeth/28/01.png"')
        div.row-down.row
            div.row--rtl
                img.t31(:src='preload', v-img='"/imgs/teeth/31/01.png"')
                img.t32(:src='preload', v-img='"/imgs/teeth/32/01.png"')
                img.t33(:src='preload', v-img='"/imgs/teeth/33/01.png"')
                img.t34(:src='preload', v-img='"/imgs/teeth/34/01.png"')
                img.t35(:src='preload', v-img='"/imgs/teeth/35/01.png"')
                img.t36(:src='preload', v-img='"/imgs/teeth/36/01.png"')
                img.t37(:src='preload', v-img='"/imgs/teeth/37/01.png"')
                img.t38(:src='preload', v-img='"/imgs/teeth/38/01.png"')
            div.row--ltr
                img.t31(:src='preload', v-img='"/imgs/teeth/41/01.png"')
                img.t32(:src='preload', v-img='"/imgs/teeth/42/01.png"')
                img.t33(:src='preload', v-img='"/imgs/teeth/43/01.png"')
                img.t34(:src='preload', v-img='"/imgs/teeth/44/01.png"')
                img.t35(:src='preload', v-img='"/imgs/teeth/45/01.png"')
                img.t36(:src='preload', v-img='"/imgs/teeth/46/01.png"')
                img.t37(:src='preload', v-img='"/imgs/teeth/47/01.png"')
                img.t38(:src='preload', v-img='"/imgs/teeth/48/01.png"')
div.control-btn-down.control-btn-down--previous previous
div.control-btn-down.control-btn-down--export export
div.control-btn-up
    img.control-btn-up-logo(src='/imgs/itp_Logo.png')
    div.vitality.control-btn-up-block
        div.control-btn-up-text
            p Vita-
            p lity
        img(src='/imgs/buttons_on.png')
    div.perio-chart.control-btn-up-block
        div.control-btn-up-text
            p Perio-
            p chart
        img(src='/imgs/buttons_on.png')



</template>

<script>
import { getQuestions } from '../vuex/getters'

export default {
    vuex: {
        getters: {
            questions: getQuestions
        }
    },
    data() {
        return {
            preload: 'http://design.ubuntu.com/wp-content/uploads/logo-ubuntu_su-white-hex-140x140.png',
            test: [
                '/imgs/teeth/11/01.png',
                '/imgs/teeth/11/02.png'
            ],
            test2: [
                '/imgs/teeth/12/01.png',
                '/imgs/teeth/12/02.png'
            ],
            current: 0,
            current2: 0
        }
    },
    ready() {
        console.log(this.questions)
    }

}
</script>

<style lang="stylus" scoped>
/*@import '/resources/assets/stylus/teeth.styl'*/
total-width-1 = 600
total-width-2 = 600
total-width-3 = 600
total-width-4 = 600
.control-btn-up-text
    font-size 30px
    line-height 20px
    font-weight bold
    letter-spacing 8px
    text-align right

.control-btn-up
    position fixed
    top 20px
    right 40px
    height 100px
    display flex
    flex-direction row
    align-items center
    img.control-btn-up-logo
        width 60px
        margin-right 15px
    .control-btn-up-block
        display flex
        align-items center
        img
            cursor pointer
            width 35px
    .vitality
        margin-right 15px

div.export-graph
    width 100%
    padding 5%
    img
        &.t11
            width percentage(75 / total-width-1)
        &.t12
            width percentage(61 / total-width-1)
        &.t13
            width percentage(65 / total-width-1)
        &.t14
            width percentage(66 / total-width-1)
        &.t15
            width percentage(64 / total-width-1)
        &.t16
            width percentage(100 / total-width-1)
        &.t17
            width percentage(87 / total-width-1)
        &.t18
            width percentage(82 / total-width-1)

        &.t21
            width percentage(75 / total-width-2)
        &.t22
            width percentage(61 / total-width-2)
        &.t23
            width percentage(64 / total-width-2)
        &.t24
            width percentage(67 / total-width-2)
        &.t25
            width percentage(63 / total-width-2)
        &.t26
            width percentage(100 / total-width-2)
        &.t27
            width percentage(87 / total-width-2)
        &.t28
            width percentage(83 / total-width-2)

        &.t31
            width percentage(46 / total-width-3)
        &.t32
            width percentage(49 / total-width-3)
        &.t33
            width percentage(55 / total-width-3)
        &.t34
            width percentage(64 / total-width-3)
        &.t35
            width percentage(66 / total-width-3)
        &.t36
            width percentage(95 / total-width-3)
        &.t37
            width percentage(96 / total-width-3)
        &.t38
            width percentage(129 / total-width-3)

        &.t41
            width percentage(46 / total-width-4)
        &.t42
            width percentage(49 / total-width-4)
        &.t43
            width percentage(55 / total-width-4)
        &.t44
            width percentage(64 / total-width-4)
        &.t45
            width percentage(66 / total-width-4)
        &.t46
            width percentage(94 / total-width-4)
        &.t47
            width percentage(97 / total-width-4)
        &.t48
            width percentage(129 / total-width-4)
div.row
    display flex
    flex-direction row
    max-height 200px
    div.row--rtl
        display flex
        flex-direction row-reverse
    div.row--ltr
        display flex
        flex-direction row

.control-btn-down
    position fixed
    bottom 20px
    font-size 50px
    font-weight bold
    cursor pointer
    &.control-btn-down--previous
        left 40px
    &.control-btn-down--export
        right 40px
    &:hover
        text-shadow -1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 1px 1px 0 #000
        color white
</style>
